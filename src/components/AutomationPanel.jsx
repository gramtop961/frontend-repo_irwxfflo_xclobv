  ...
  const exportToSheet = async () => {
    ...
    try {
      const res = await fetch(`${backend}/api/export-to-sheet`, {
        ...
        body: JSON.stringify({ webhook_url: webhook, range_days: Number(rangeDays) })
      })
  ...
  const sendWhatsApp = async () => {
    ...
    try {
      const res = await fetch(`${backend}/api/whatsapp/send-schedule`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ recipient_phone: recipient, message })
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.detail || 'Failed')
  ...
  return (
    <div className="bg-slate-800/60 border border-blue-500/20 rounded-xl p-4 space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="text-blue-200/80 text-sm mb-2 inline-flex items-center gap-2"><Phone className="w-4 h-4"/> Send schedule via WhatsApp</div>
