...
export default function CalendarView() {
  ...
  const colors = (ev) => ev?.source?.color || '#60a5fa'
    ...
    <div className="bg-slate-800/60 border border-blue-500/20 rounded-xl p-4">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {events.map(ev => (
            <div key={ev.id} className="rounded-lg p-3" style={{backgroundColor: colors(ev)+"22", border:`1px solid ${colors(ev)}55`}}>
              <div className="text-xs text-blue-200/70">{ev.source?.name || 'Source'}</div>
